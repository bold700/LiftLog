...
/**
 * Het gesprek blijft bewaard zolang de app open is (sessionStorage, per gebruiker): wie even naar
 * Workouts kijkt en terugkomt, vond eerst een leeg scherm. Bij sluiten van de app begint het opnieuw.
 */
const chatKey = (uid: string | undefined) => `liftlog.assistant.${uid ?? 'anon'}`;

function loadChat(uid: string | undefined): ChatItem[] {
  try {
    const raw = sessionStorage.getItem(chatKey(uid));
...
export function AssistentPage() {
  ...
  const [items, setItems] = useState<ChatItem[]>(() => loadChat(uid));

  useEffect(() => {
    try {
      if (items.length) sessionStorage.setItem(chatKey(uid), JSON.stringify(items));
      else sessionStorage.removeItem(chatKey(uid));
    } catch {
      /* privémodus: dan alleen in het geheugen */
    }
  }, [items, uid]);
  ...
  return (
    // Links uitgelijnd met de paginatitel (zoals de andere schermen), niet gecentreerd; leesbaar tot 800px breed.
    <PageLayout maxWidth="none">
      <Box sx={{ maxWidth: designTokens.pageMaxWidth }}>
        ...
        <ContentCard>
          <Box
            sx={{
              ...
              gap: 1.5,
              // Telefoon: het gesprek vult het scherm tot net boven de zwevende toolbar (invoer blijft vrij); desktop: vaste hoogte.
              minHeight: { xs: 'max(240px, calc(100dvh - 520px))', md: 280 },
              maxHeight: { xs: 'max(240px, calc(100dvh - 520px))', md: '60vh' },
              overflowY: 'auto',
              pr: 0.5,
            }}
          >
