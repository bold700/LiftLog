...
describe('lesrooster: momenten uit het schema', () => {
  it('levert precies één moment per week voor één weekmoment', () => {
    const schedule = [{ weekday: 4, startTime: '19:00' }]; // donderdag
    const out = occurrencesForSchedule(schedule, THURSDAY, 3);
    expect(out).toEqual([
      { date: '2026-09-24', startTime: '19:00' },
      { date: '2026-10-01', startTime: '19:00' },
      { date: '2026-10-08', startTime: '19:00' },
    ]);
  });

  it('telt vandaag mee als het de juiste weekdag is', () => {
    const out = occurrencesForSchedule([{ weekday: 4, startTime: '19:00' }], THURSDAY, 1);
    expect(out[0].date).toBe(THURSDAY);
  });

  it('combineert meerdere weekmomenten van dezelfde lessoort', () => {
    const schedule = [
      { weekday: 4, startTime: '19:00' }, // donderdag
      { weekday: 1, startTime: '18:00' }, // maandag
    ];
    ...
    expect(out).toEqual([
      { date: '2026-09-24', startTime: '19:00' },
      { date: '2026-09-28', startTime: '18:00' },
      { date: '2026-10-01', startTime: '19:00' },
      { date: '2026-10-05', startTime: '18:00' },
    ]);
  });
...
describe('lesrooster: wat nog ontbreekt', () => {
  it('laat een moment weg zodra het al bestaat (gemaakt of afgelast, maakt niet uit)', () => {
    ...
    expect(out).toEqual([{ date: '2026-10-01', startTime: '19:00' }]);
  });
