...
describe('heartRateZones', () => {
  it('zonder rusthartslag: percentage van max', () => {
    ...
    expect(z?.zones[4]).toMatchObject({ zone: 5, lowBpm: 168, highBpm: 186 });
  });
  it('met rusthartslag: Karvonen (rust + reserve × %)', () => {
    ...
    // reserve = 124; zone 2 = 62 + 124 × 0,6 … 0,7 (ondergrens één boven zone 1)
    expect(z?.zones[1]).toMatchObject({ zone: 2, lowBpm: 137, highBpm: 149 });
  });
  it('zones sluiten op elkaar aan zonder overlap', () => {
    const z = heartRateZones(45, 55)!;
    for (let i = 1; i < z.zones.length; i++) expect(z.zones[i].lowBpm).toBe(z.zones[i - 1].highBpm + 1);
  });
